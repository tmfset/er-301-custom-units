...
local SloopFlagControl = Class {}
SloopFlagControl:include(MenuControl)

function SloopFlagControl:init(args)
  MenuControl.init(self)
  self:setClassName("Sloop.SloopFlagControl")
  ...
  local flagsPad = args.flagsPadding or 0
  ...
  self.flagsPad = flagsPad

  local graphic = app.RichTextBox(description, 10)
  -- graphic:setBorder(1)
  -- graphic:setBorderColor(app.GRAY7)
  -- graphic:setCornerRadius(3, 3, 3, 3)
  graphic:setMargins(3, 1, (#flags + flagsPad) * ply, 1)
  graphic:fitHeight((#flags + flagsPad + descWidth) * ply - 2)
...
function SloopFlagControl:onReleased(i, shifted)
...
function SloopFlagControl:update()
