...
function OutputMeter:init(args)
  Base.init(self)
  self:setClassName("polygon.OutputMeter")
  ...
  local polygon = args.polygon or app.logError("%s.init: polygon is missing.", self)
  ...
  self.fader = (function ()
    ...
    fader:setParameter(polygon:getParameter("Output Gain"))
  ...
  local readout = (function ()
    local param = polygon:getParameter("AGC")
  ...
  self.subGraphic:addChild(self.agcIndicator)

  self.inputGain = (function ()
    local param = polygon:getParameter("Output Gain")
...
function OutputMeter:getLeftWatch()
  return self.polygon:getOutput("Out1")
end
...
function OutputMeter:getRightWatch()
  ...
  return self.polygon:getOutput("Out2")
end

function OutputMeter:updateViewState()
  if self.polygon:isAgcEnabled() then
    self.agcIndicator:on()
  else
    self.agcIndicator:off()
  end
end
